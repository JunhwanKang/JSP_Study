package namecard.controller;

import java.io.IOException;

import javax.servlet.RequestDispatcher;
import javax.servlet.ServletException;
import javax.servlet.annotation.WebServlet;
import javax.servlet.http.HttpServlet;
import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;

import namecard.dao.NamecardDao;
import namecard.entity.Namecard;

@WebServlet("/namecard/update")
public class NamecardUpdateServlet extends HttpServlet {
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		int num = Integer.parseInt(request.getParameter("num"));
		Namecard namecard = NamecardDao.findById(num);
		request.setAttribute("namecard", namecard);
		
		RequestDispatcher rd = request.getRequestDispatcher("/namecard/update.jsp");
		rd.forward(request, response);
	}
	
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		request.setCharacterEncoding("utf-8");
		String name = request.getParameter("name");
		String email = request.getParameter("email"); 
		String tel = request.getParameter("tel"); 
		int num = Integer.parseInt(request.getParameter("num"));
		
		Namecard namecard = new Namecard(num, name, email, tel);
		NamecardDao.update(namecard);
		response.sendRedirect("/move_example/namecard/list");
	}
}
